...
///      for (j=1; j<N-1; j++) {
///        p[i][j] = -c / (a*p[i][j-1]+b);
///        q[i][j] = (-d*u[j][i-1]+(1.0f+2.0f*d)*u[j][i] -
...
///      for (j=1; j<N-1; j++) {
///        p[i][j] = -f / (d*p[i][j-1]+e);
///        q[i][j] = (-a*v[i-1][j]+(1.0f+2.0f*a)*v[i][j] -
...
\
  Real_type DX = 1.0f/(Real_type)n; \
  Real_type DY = 1.0f/(Real_type)n; \
  Real_type DT = 1.0f/(Real_type)tsteps; \
  Real_type B1 = 2.0f; \
  Real_type B2 = 1.0f; \
  Real_type mul1 = B1 * DT / (DX * DX); \
  Real_type mul2 = B2 * DT / (DY * DY); \
  Real_type a = -mul1 / 2.0f; \
  Real_type b = 1.0f + mul1; \
  Real_type c = a; \
  Real_type d = -mul2 /2.0f; \
  Real_type e = 1.0f + mul2; \
  Real_type f = d; \
\
  Real_ptr U = m_U; \
  Real_ptr V = m_V; \
...
#define POLYBENCH_ADI_BODY3 \
  ...
  Q[i * n + j] = (-d * U[j * n + i-1] + (1.0f + 2.0f*d) * U[j * n + i] - \
...
#define POLYBENCH_ADI_BODY7 \
  ...
  Q[i * n + j] = (-a * V[(i-1) * n + j] + (1.0f + 2.0f*a) * V[i * n + j] - \
...
#define POLYBENCH_ADI_BODY3_RAJA \
  ...
  Qview(i, j) = (-d * Uview(j, i-1) + (1.0f + 2.0f*d) * Uview(j, i) - \
...
#define POLYBENCH_ADI_BODY7_RAJA \
  ...
  Qview(i, j) = (-a * Vview(i-1, j) + (1.0f + 2.0f*a) * Vview(i, j) - \
